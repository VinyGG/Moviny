...
if ($_POST['searchword'] == 'bestnowatched') {
	$sql = "SELECT * FROM `movie` WHERE `mov_watched` LIKE '0' ORDER BY `mov_imdb_rating` DESC";
	$query = mysqli_query($mysqli, $sql);
	$rows = mysqli_num_rows($query);

	if ($rows > 0) {
		echo '<tr><td></td><td align="center" style="color:#990000">' . $rows . " filme(s) encontado(s)</td><td></td></tr>";
	...
	$sql = "SELECT * FROM `movie` WHERE (`mov_watched` = '1' AND `mov_my_rating` >= 7 AND `mov_my_rating` <= 10) ORDER BY `mov_my_rating` DESC";
	$query = mysqli_query($mysqli, $sql);
	$rows = mysqli_num_rows($query);
